...
.data 
  ...
  p_int:        .asciiz "\n Nhap so thu nhat co 3 chu so.\nNhap 021 de lay so 21.\n"
...
start:
	la $a0,welc		# Print Welcome
	li $v0, 4     
	syscall

	la $a0,p_int		# print message input first number
	li $v0, 4     
	syscall
#----------------------------------------------------------------------
# Enable interupt 
...
#----------------------------------------------------------------------
	...
	li $s3,0		# $s3: =
	li $s5,0		# first number = 0
	li $s6,0		# second number = 0
	li $s4,0		# counter s4 = 0
#---------------------------------------
# Declare :
#	$s7 : result of calculation
#	$t8, $t9 : hai chu so cuoi cua ket qua. VD :kq = 123 thi $t8 = 2, $t9 = 3 
#	$s0 : nap gia tri tu $t8, $t9 de hien thi
...
#	beq $s4,3,nhapso2    # Wait for interrupt
#	nop
#	beq $s4,4,nhapso2
#	b Loop
...
nhapso2:
	...
	add $s1,$0,$0	# reset s1
	addi $s4,$0,0	# reset counter s4 = 0
	
	la $a0,xuongdong
 	li $v0, 4     
  	syscall
	
	move $a0,$s5 	# print first number
	li $v0,1
	syscall 
  
	li $t6,0      	# reset left LED
	li $t7,0 	# reset right LED

	la $a0,p_toantu	# print message input second number
	li $v0,4
	syscall
#----------------------------------------------------------------------
Loop2:
...
nhaptoantu:
	la $a0,p_int1	# print message input second number
	li $v0,4
	syscall
 ...
 # 	la $a0,p_toantu		# print request input +/-/*//
  #	li $v0,4
  #	syscall
#----------------------------------------------------------------------
Loop1: 
	# Vong lap cho interrupt so thu 2
	beq $s4,3,luuGiatri2
	nop
	beq $s4,3,luuGiatri2
	nop
	...
	la $a0,xuongdong
 	li $v0, 4     
  	syscall
   	...
   	move $a0,$s6  		# print second number
  	li $v0,1
  	syscall 
...
showketqua: li $t9,0
            ...
            bne $k1,$0,loisoam
            div $t8,$s7,10	# result s7/10 => hi = so du, t8 = lo = thuong
            mfhi $t9		# Move from hi to $t9
            beq $t8,0,napgiatricholed

            div $t8,$t8,10	# if t8 = lo > 10; -> gaÌ?n t8 = hi
            mfhi $t8
...
endmain:
       	addi $v0, $zero, 51		# dialog continue or exit
       	la $a0, rep
       	syscall
       	
       	beq $a0, $0, END	# if input = 0 -> end
       	nop
...
# s0 = t8 hoac s0 = t9
# s0 = 0, 1, 2... thi gan byte cua ZERO, ONE, TWO cho t4
# luu 1 byte cua t4 -> t2
...
back_main:  eret
  ...
  # Tra ve:
  #------------------------------------------------------------------------
 #  $s1 la ket qua cuoi cung
 # $s2 la toan tu lay ra, 1 +; 2-; 3 *; 4/ ;  f6 =
getInt1: 
	addi $sp,$sp,4
      	sw $ra,0($sp) 
      	li $t1,IN_ADDRESS_HEXA_KEYBOARD
...
case_0:   
	li $t5,0x11	    # Gan t5 = dia chi cua 0
      	...
      	j show1
case_1:   
	li $t5,0x21	    # Gan t5 = dia chi cua 1
...
getInt2: 
	addi $sp,$sp,4
      	...
      	sb $t3,0($t1)
      	li $t1,OUT_ADDRESS_HEXA_KEYBOARD	# luu dia chi duoc bam vao $t1
      	lb $t3,0($t1)   # Nhan byte the hien vi tri cua phim duoc bam trong hang 1
case_4:   
	li $t5,0x12
